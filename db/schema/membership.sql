...
BEGIN
    ...
    select row_to_json(t) INTO output_json
    from (select return_id::TEXT, new_session_id::TEXT, message, pkey, success, return_name) t;
...
BEGIN
    ...
    elseif (trim(return_email) = '') THEN 
        select 0 into new_id;
        select 'Please enter a valid email' into message;
    elseif (trim(pass) = '') THEN 
        select 'Please enter a password' into message;      
    ELSE
        select true into success;
        --encrypt password
    ...
    select row_to_json(t) into output_json
    from (
      select new_id::TEXT, message, new_email, success, return_status, validation_token::TEXT
    ) t;
